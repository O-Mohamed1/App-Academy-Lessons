...
// made this faster code for returning multiples, don't know how big O notation
// works but this has gotta be way better than the other version
// function returnMultiples(max,divisor) {
//     if (max % divisor !== 0) {
//       max - max % divisor
//     }
//     let num = max / divisor
...
function countConsonants(word) {
    const unwantedChars = ["a", "e", "i", "o", "u", " "];
    let count = word.length;
    for (let i = 0; i < word.length; i++) {
        if (unwantedChars.includes(word[i])) {
            count--;
        }
    }
    return count;
...
function alternatingLetters(str) {
    ...
    for (let i=0;i<chars.length;i++){
        if (i%2===0){
            altStr.push(chars[i].toLowerCase())
        }else{altStr.push(chars[i].toUpperCase())}
    }
    altStr=altStr.join("")
    return altStr;
}
